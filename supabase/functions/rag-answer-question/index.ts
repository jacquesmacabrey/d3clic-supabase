...
const FUNCTION_VERSION = "RAG-5-DETERMINISTIC-AUDIT-2026-07-27";
...
function normalizeQuestion(value: unknown): string {
  ...
  if (
    normalized.length < 3 ||
    normalized.length > MAX_QUESTION_CHARACTERS
  ) {
    throw new RagError("invalid_request", "Question invalide.", 400);
  }
  return normalized;
...
async function parseQuestion(request: Request): Promise<string> {
  ...
  return normalizeQuestion(body.question);
}
...
Deno.serve(async (request: Request) => {
  ...
  try {
    ...
    if (noSources) {
      ...
      log("completed", 200);
      return jsonResponse(request, requestId, noSources.client);
    }
    ...
    if (context.included.length === 0) {
      ...
      log("completed", 200);
      return jsonResponse(request, requestId, result.client);
    }
