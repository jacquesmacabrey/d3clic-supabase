...
export function extractQuestionFacts(question: string): FactExtraction {
  ...
  const serviceValues = collectNumbers(text, [
    /\b(-?\d+(?:[.,]\d+)?)\s*(?:ans?|annees?)\s+(?:de\s+service|d[' ]anciennet[eé])\b/g,
    /\b(?:anciennet[eé]|service)\s*(?:de|:|est\s+de)?\s*(-?\d+(?:[.,]\d+)?)\s*(?:ans?|annees?)\b/g,
    /\b(?:compte|ayant|avec)\s+(-?\d+(?:[.,]\d+)?)\s*(?:ans?|annees?)\s+(?:de\s+service|d[' ]anciennet[eé])\b/g,
...
export function evaluateValidatedRuleSets(
  ...
  const matchingRuleSets = ruleSets.filter(
    (ruleSet) => ruleSet.ruleKey === "annual_leave_days",
  );
  ...
  const ruleSet = matchingRuleSets[0];
